...
export function ActHero() {
  ...
  // Cursor-lit grid: track the pointer over the hero as CSS vars the gold grid
  // layer is masked to. Desktop / fine-pointer only.
  useEffect(() => {
  ...
  const line = (delay: string): CSSProperties => ({ ["--line-delay" as string]: delay });
  ...
  return (
    ...
    >
      ...
      <motion.div style={{ y: gridY }} className="pointer-events-none absolute inset-0 opacity-[0.5]">
        ...
        />
        {/* gold layer that lights up around the cursor */}
        <div className="hero-grid-lit" aria-hidden />
      </motion.div>
      ...
      <div className="relative z-10 pb-12 md:pb-16 px-6 md:px-10 mx-auto max-w-[1600px] w-full flex flex-col md:flex-row md:items-end justify-between gap-10">
        ...
        <div className="rise flex items-end gap-8" style={{ animationDelay: "0.7s" }}>
          ...
          <div
            data-magnetic
            className="shrink-0 w-20 h-20 md:w-24 md:h-24 rounded-full border border-gold text-gold flex items-center justify-center text-center label text-[8px] leading-[1.6]"
          >
            made.<br />est.<br />2026
          </div>
        </div>
